...
def load_checkpoint(
    ...
    if convert_from_caffe2:
        ...
        if inflation:
            ...
            load_orvit_attn = epoch_reset and load_orvit_attn_from_bb
            ...
            if load_orvit_attn:
                qkv_keys = [k for k in checkpoint["model_state"].keys() if k.startswith('blocks') and 'qkv' in k]
                sd = ms.state_dict().keys()
                for k in qkv_keys:
                    for kk in [k, f'orvit_{k}']:
                        if kk in sd:
                            logger.info(f'Loading {k} as {kk} for orvit layer')
                            pre_train_dict[kk] = checkpoint["model_state"][k]
...
def load_test_checkpoint(cfg, model):
    ...
    if cfg.TEST.CHECKPOINT_FILE_PATH != "":
        ...
        cur_epoch = load_checkpoint(last_checkpoint, model, cfg.NUM_GPUS > 1)
        return cur_epoch
        ...
        cur_epoch = load_checkpoint(
            ...
            load_orvit_attn_from_bb=cfg.ORVIT.ENABLE and cfg.ORVIT.LOAD_ORVIT_ATTN_LAYERS_FROM_BB,
            should_split_qkv = cfg.SPLIT_QKV_CHECKPOINT,
        )
        return cur_epoch
    else:
