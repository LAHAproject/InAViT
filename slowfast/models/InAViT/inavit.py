...
class InAViT(nn.Module):
    ...
    def __init__(
        ...
        self.with_motion_stream = cfg.HOIVIT.USE_MOTION_STREAM

        self.maskfg = cfg.HOIVIT.MASK_FG
        ...
        if self.cfg.HOIVIT.HANDONLY:
            self.box_categories = nn.Parameter(torch.zeros(self.nb_frames, self.cfg.HOIVIT.U, self.in_dim))
        elif self.cfg.HOIVIT.OBJONLY:
            self.box_categories = nn.Parameter(torch.zeros(self.nb_frames, self.cfg.HOIVIT.O, self.in_dim))
        else:
        ...
        self.hand_mask_weights = nn.Parameter(torch.ones(self.cfg.HOIVIT.U))
        self.obj_mask_weights = nn.Parameter(torch.ones(self.cfg.HOIVIT.O))
    ...
    def forward(self, x, metadata,thw):
        ...
        for u in range(U):
            hand_obj_masks = []
            for o in range(O):
                m_o = torch.sum(torch.cdist(hand_traj[:,:,u], obj_traj[:,:,o], p=2), dim=-1)
                if len(m_o.shape) > 1:
                    m_o = torch.sum(m_o, dim=-1)
                m_o = 1 / ((m_o // T))
                m_o = torch.nan_to_num(m_o, nan=1.0, posinf=1.0)
                hand_obj_masks.append(m_o)    
            hand_obj_masks = torch.stack(hand_obj_masks)*self.obj_mask_weights
            hand_mask.append(hand_obj_masks)
        hand_mask = torch.mean(torch.cat(hand_mask),dim=0)

        obj_mask = []
        for o in range(O):
            obj_hand_masks = []
            for u in range(U):
                m_u = torch.sum(torch.cdist(hand_traj[:,:,u], obj_traj[:,:,o], p=2), dim=-1)
                if len(m_u.shape) > 1:
                    m_u = torch.sum(m_u, dim=-1)
                m_u = 1 / ((m_u // T))
                m_u = torch.nan_to_num(m_u, nan=1.0, posinf=1.0) # replace NaN, inf by 1.0
                obj_hand_masks.append(m_u)
            obj_hand_masks = torch.stack(obj_hand_masks)*self.hand_mask_weights
            obj_mask.append(obj_hand_masks)

        obj_mask = torch.mean(torch.cat(obj_mask),dim=0)
        '''
        if self.maskfg and get_patch_orig:
                patch_tokens_orig = x_orig.permute(0,2,1).reshape(BS, -1, T,H,W)
                obj_tokens = self.crop_layer(patch_tokens_orig, obj_box_tensors)  # [BS, O,T, d, H, W]
                hand_tokens = self.crop_layer(patch_tokens_orig, hand_box_tensors)  # [BS, U,T, d, H, W]
        else:    
        ...
        hand_tokens = hand_tokens.reshape(BS, T, O, H*W, d)
        ...
        if self.cfg.HOIVIT.HANDONLY:
            hand_tokens_mod = torch.cat(hand_tokens_mod).reshape(BS, T, U, d)
            hand_obj_tokens = hand_tokens_mod
            box_categories = self.box_categories.unsqueeze(0).expand(BS,-1,-1,-1)
            box_emb = self.c_coord_to_feature(hand_box_tensors.float())
            hand_obj_tokens = hand_obj_tokens + box_categories + box_emb # [BS, T, U, d]
        elif self.cfg.HOIVIT.OBJONLY:
            obj_tokens_mod = torch.cat(obj_tokens_mod).reshape(BS, T, O, d)
            hand_obj_tokens = obj_tokens_mod  
            box_categories = self.box_categories.unsqueeze(0).expand(BS,-1,-1,-1)
            box_emb = self.c_coord_to_feature(obj_box_tensors.float())
            hand_obj_tokens = hand_obj_tokens + box_categories + box_emb # [BS, T, O, d]
        else:
            hand_tokens_mod = torch.cat(hand_tokens_mod).reshape(BS, T, U, d)
            obj_tokens_mod = torch.cat(obj_tokens_mod).reshape(BS, T, O, d)
        ...
        if self.with_cls_token:
            ...
            if self.cfg.HOIVIT.DOUBLEATTENTION:
                patch_tokens = torch.cat([cls_token, patch_tokens.permute(0,2,3,4,1).reshape(BS, T, H*W, d).flatten(1,2)], dim = 1) # [BS, 1 + T*N, d]
                hand_obj_tokens = torch.cat([cls_token, hand_obj_tokens.flatten(1,2)], dim = 1)  # [BS, 1 + T*(O+U), d]
        ...
        if self.cfg.HOIVIT.DOUBLEATTENTION:
            patch_tokens, _ = self.timecrossattn(
                        ...
                        self.norm2(hand_obj_tokens),
                        [T, H*W, 1]
                    )
            cls_token_new, patch_tokens =  patch_tokens[:, [0]], patch_tokens[:, 1:]
            cls_token = 0.5 * (cls_token + cls_token_new)
            if self.cfg.HOIVIT.HANDONLY:
                patch_tokens = 0.5 * (patch_tokens + all_tokens.reshape(BS,T,H*W+U,d)[:,:,:H*W].reshape(BS,T*H*W,d))
            elif self.cfg.HOIVIT.OBJONLY:
                patch_tokens = 0.5 * (patch_tokens + all_tokens.reshape(BS,T,H*W+O,d)[:,:,:H*W].reshape(BS,T*H*W,d))
            else:
                # patch_tokens = 0.5 * (patch_tokens + all_tokens.reshape(BS,T,H*W+O+U,d)[:,:,:H*W].reshape(BS,T*H*W,d))
                # for revtca
                patch_tokens = torch.cat((patch_tokens,all_tokens.reshape(BS,T,H*W+O+U,d)[:,:,:H*W].reshape(BS,T*H*W,d)),dim=1)
        else:
            patch_tokens = all_tokens.reshape(BS,T,H*W+O+U,d)[:,:,:H*W].reshape(BS,T*H*W,d)

        if self.with_motion_stream:
            if self.cfg.HOIVIT.HANDONLY:
                box_tensors = hand_box_tensors
            elif self.cfg.HOIVIT.OBJONLY:
                box_tensors = obj_box_tensors
            else:
