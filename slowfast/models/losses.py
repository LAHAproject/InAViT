...
class EGTEALoss(nn.Module):
    ...
    def forward(self, extra_preds, y):
        verb = extra_preds['verb']
        noun = extra_preds['noun']
        action = extra_preds['action']
        verb_loss = self.ce_loss(verb, y['verb'])
        noun_loss = self.ce_loss(noun, y['noun'])
        action_loss = self.ce_loss(action, y['action'])
        return {'verb_loss':verb_loss,  'noun_loss':noun_loss, 'action_loss':action_loss}
...
def get_loss_func(cfg, state = 'train'):
    ...
    if cfg.TRAIN.DATASET == 'epickitchens':
        ...
        if loss_name == 'cross_entropy':
            ret = partial(EGTEALoss, ce_type='')
        elif loss_name == 'soft_cross_entropy':
            ret = partial(EGTEALoss, ce_type='soft')
        elif loss_name == 'label_smoothing_cross_entropy':
            ret = partial(EGTEALoss, ce_type='label_smoothing', smoothing=cfg.MIXUP.LABEL_SMOOTH_VALUE)
        else:
            raise NotImplementedError(f"{loss_name} for epickitchens")
    else:
