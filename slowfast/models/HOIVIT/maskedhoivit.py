...
class MaskedHOIViT(nn.Module):
    ...
    def __init__(self, cfg):
        ...
        if self.with_motion_stream:
            self.motion_stream = MotionStream(cfg, dim=dim, num_heads=self.num_heads, attn_type=cfg.ORVIT.MOTION_STREAM_ATTN_TYPE, 
                                                mlp_ratio=mlp_ratio, qkv_bias=self.qkv_bias, drop=self.drop_rate, attn_drop=self.attn_drop_rate, 
                                                drop_path=drop_path, act_layer=act_layer, norm_layer=norm_layer,
                                                nb_frames=self.temporal_resolution,
                                                )
            self.motion_mlp = AddObjDimMlp(in_features=cfg.ORVIT.MOTION_STREAM_DIM if cfg.ORVIT.MOTION_STREAM_DIM > 0 else dim,
                                        hidden_features=mlp_hidden_dim, out_features=dim,
                                        act_layer=act_layer, drop=self.drop_rate, num_objects=self.cfg.HOIVIT.O+self.cfg.HOIVIT.U, 
                                        T= self.temporal_resolution, H=cfg.DATA.TRAIN_CROP_SIZE // cfg.MF.PATCH_SIZE, W=cfg.DATA.TRAIN_CROP_SIZE // cfg.MF.PATCH_SIZE)
    ...
    def forward_features(self, x, obj_box_tensors, hand_box_tensors): # x: [BS, C=3, T=16, H=224, W=224]
        ...
        x_orig = x.clone()
        # Tokenize input
        x_orig = self.patch_embed_3d(x_orig) # [BS, N=T'*H'*W', d]
    ...
    def forward(self, x, metadata): # x: [BS, C=3, T=16, H=224, W=224]
        ...
        x, x_orig, thw = self.forward_features(x, obj_box_tensors, hand_box_tensors) # [BS, d]

        if self.with_cls_token:
            patch_tokens_orig = x_orig
            cls_token, patch_tokens = x[:,[0]], x[:,1:]

        BS, _, d = x_orig.shape
        T,H,W = thw
        patch_tokens_orig = patch_tokens_orig.permute(0,2,1).reshape(BS, -1, T,H,W).detach()
        ...
        # all_tokens = all_tokens.reshape(BS,T,H*W+O+U,d)[:,:,:H*W].reshape(BS,T*H*W,d) # Select only H*W patches
        
        if self.with_motion_stream:
            box_tensors = torch.cat((obj_box_tensors, hand_box_tensors), dim=-2)
            motion_emb = self.motion_stream(box_tensors,H, W) # [BS, T*H*W, d]
            motion_emb = self.motion_mlp(motion_emb) # [BS, T*(H*W+O+U), d]
            all_tokens = all_tokens + motion_emb
...
class AddObjDimMlp(nn.Module):
    def __init__(
        self, in_features, hidden_features=None, 
        out_features=None, act_layer=nn.GELU, drop=0., 
        num_objects=1, T=8, H=14, W=14
    ):
        super().__init__()
        out_features = out_features or in_features
        hidden_features = hidden_features or in_features
        self.add_obj_dim = nn.Linear(T*H*W, T*(H*W+num_objects))
    ...
    def forward(self, x):
        x = x.permute(0,2,1)# BS, T*H*W, d -> BS, d, T*H*W
        x = self.add_obj_dim(x) # BS, d, T*H*W -> BS, d, T*(H*W+O+U)
        x = self.drop(x)
        x = x.permute(0,2,1)# BS, d, T*(H*W+O+U) -> BS, T*(H*W+O+U), d 
        x = self.fc1(x)
        x = self.act(x)
        x = self.drop(x)
        x = self.fc2(x)
