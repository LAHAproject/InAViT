...
from slowfast.datasets.boxes_autoaugment.bbox_util import clip_box
from slowfast.utils.box_ops import box_cxcywh_to_xyxy

def retry_load_images(image_paths, cfg, retry=10, backend="pytorch"):
    """
    ...
    if spatial_idx == -1:
        if aspect_ratio is None and scale is None:
            if isinstance(boxes, dict):
                obj_boxes = boxes['obj']
                frames, obj_boxes = transform.random_short_side_scale_jitter(
                    images=frames,
                    min_size=min_scale,
                    max_size=max_scale,
                ...
                frames, obj_boxes = transform.random_crop(frames, crop_size, boxes=obj_boxes)
                boxes['obj'] = obj_boxes
            else:
        ...
        if random_horizontal_flip:
            if isinstance(boxes, dict): 
                obj_boxes = boxes['obj']
                frames, obj_boxes = transform.horizontal_flip(0.5, frames, boxes=obj_boxes)
                boxes['obj'] = obj_boxes
            else:
        ...
        if isinstance(boxes, dict): 
            obj_boxes = boxes['obj']
            frames, obj_boxes = transform.random_short_side_scale_jitter(
                frames, min_scale, max_scale, boxes=obj_boxes,
            )
            boxes['obj'] = obj_boxes
        else:
            frames, boxes = transform.random_short_side_scale_jitter(
        ...
        if isinstance(boxes, dict): 
            obj_boxes = boxes['obj']
            frames, obj_boxes = transform.uniform_crop(frames, crop_size, spatial_idx, boxes=obj_boxes)
            boxes['obj'] = obj_boxes
        else:
...
# perform foreground masking
def mask_fg(frames, boxes): # boxes = B, T, O, 4
    B, C, T, H, W = frames.shape
    frames = frames.permute(0, 2, 1, 3, 4).flatten(0, 1) # B*T, C, H, W
    
    boxes = boxes.flatten(0,1) #B, T, O, 4 -> B*T, O, 4 
    ...
    frames = masking_frames(frames, boxes)
    return frames.reshape(B,C,T,H,W) # B*T, C, H, W -> B,C, T, H, W

def masking_frames(frames, boxes):
    ...
    for i in range(frames.shape[0]): 
        box_image = boxes[i,:,:] # O,4
        im = to_pil(frames[i,:,:,:])
        for j in range(box_image.shape[0]):
            obj_box = box_image[j,:].astype(int) # 4
            clipbox = np.array([0, 0, W, H]) 
            try:
                _ = clip_box(obj_box, clipbox, alpha=0.25)
                 # C,H,W
                im = mask_frame(im, obj_box)
            except:
                # print("Object cannot be masked")
                continue
        masked_frames.append(to_tensor(im))
    return torch.stack(masked_frames)   # B*T, C, H, W
...
def mask_frame(im, box):
    # Create rounded rectangle mask
    mask = Image.new('L', im.size, 0)
    draw = ImageDraw.Draw(mask)
    rounded_rectangle(draw, box, rad=40, fill=255)
