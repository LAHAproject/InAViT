...
from .ek_MF.frame_loader import pack_frames_to_video_clip, get_video_clip, pack_frames_to_video_clip_next
...
class Epickitchens_dataset(torch.utils.data.Dataset):
    ...
    def getitem(self, index):
        ...
        next_frames, seq = pack_frames_to_video_clip_next(self.cfg, self._video_records[index], temporal_sample_index, target_fps=self.target_fps, ret_seq = True)

        frames = torch.as_tensor(frames) # [T, H, W, C]
        
        display_frames = frames.clone().detach()
        ...
        if boxes is not None:
            ...
            boxes['obj'] = self.ek_boxes.prepare_boxes(boxes['obj'], nid = nid)
            boxes['hand'] = self.ek_boxes.prepare_boxes(boxes['hand'], nid = nid)
            boxes['obj'] = boxes['obj'].clone().detach()
            boxes['hand'] = boxes['hand'].clone().detach()
            metadata['orvit_bboxes'] = boxes
        
        label_names = self._video_records[index].label_names
        return  frames, label, index, metadata, next_frames, display_frames, label_names
