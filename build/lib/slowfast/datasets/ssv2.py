...
#            sort_out = json.load(f)
        ...
        self._path_to_videos, _ = utils.ssv2_load_image_lists(
            path_to_file, prefix=self.cfg.DATA.PATH_PREFIX+'/fps_12', sort_out = sort_out,
        )
    ...
    def __getitem__(self, index):
        ...
        if self.cfg.ORVIT.ENABLE or self.cfg.HOIVIT.ENABLE or self.cfg.STDVIT.ENABLE or self.cfg.UNIONHOIVIT.ENABLE:
            fpaths, boxes, boxes_metadata = self.get_boxes(index)
            if self.cfg.HOIVIT.ENABLE or self.cfg.STDVIT.ENABLE or self.cfg.UNIONHOIVIT.ENABLE:
                ori_boxes = boxes
            else:
            ...
            if isinstance(boxes,dict):
                boxes['obj'] = boxes['obj'].numpy() # [T, O, 4], xyxy, unnormalized
                boxes['hand'] = boxes['hand'].numpy() # [T, U, 4], xyxy, unnormalized
            else:
        ...
        if self.cfg.ORVIT.ENABLE or self.cfg.HOIVIT.ENABLE or self.cfg.STDVIT.ENABLE or self.cfg.UNIONHOIVIT.ENABLE:
            ...
            if isinstance(ori_boxes, dict):
                ori_boxes['obj'][..., [0,2]] = ori_boxes['obj'][..., [0,2]] / _w 
                ori_boxes['obj'][..., [1,3]] = ori_boxes['obj'][..., [1,3]] / _h
                ori_boxes['hand'][..., [0,2]] = ori_boxes['hand'][..., [0,2]] / _w 
                ori_boxes['hand'][..., [1,3]] = ori_boxes['hand'][..., [1,3]] / _h
        ...
        if boxes is not None:
            ...
            if isinstance(boxes, dict):
                boxes['obj'][..., [0,2]] = boxes['obj'][..., [0,2]] / w
                boxes['obj'][..., [1,3]] = boxes['obj'][..., [1,3]] / h
                boxes['hand'][..., [0,2]] = boxes['hand'][..., [0,2]] / w
                boxes['hand'][..., [1,3]] = boxes['hand'][..., [1,3]] / h
                boxes['obj'] = np.clip(boxes['obj'], 0, 1)
                boxes['hand'] = np.clip(boxes['hand'], 0, 1)
                boxes['obj'] = torch.from_numpy(boxes['obj']) # [T, O, 4]
                boxes['hand'] = torch.from_numpy(boxes['hand']) # [T, U, 4]
                boxes['obj'] = box_ops.box_xyxy_to_cxcywh(boxes['obj'])
                boxes['hand'] = box_ops.box_xyxy_to_cxcywh(boxes['hand'])
                boxes['obj'] = box_ops.zero_empty_boxes(boxes['obj'], mode='cxcywh')
                boxes['hand'] = box_ops.zero_empty_boxes(boxes['hand'], mode='cxcywh')
            else:
    ...
    def _aug_frame(
        ...
        if self.cfg.AUG.DIFFERENT_AUG_PER_FRAME:
            if isinstance(boxes, dict):
                list_img = [get_rand_aug()([img], boxes=boxes['obj'][[i]] if boxes is not None else None) for i, img in enumerate(list_img)]
            else:
                list_img = [get_rand_aug()([img], boxes=boxes[[i]] if boxes is not None else None) for i, img in enumerate(list_img)]
            if boxes is not None:
                if isinstance(boxes, dict):
                    list_img, obj_boxes = zip(*list_img)
                    boxes['obj'] = np.concatenate(obj_boxes, axis = 0)
                    boxes['hand'] = np.concatenate(boxes['hand'], axis = 0)
                else:
            ...
            if isinstance(boxes,dict):
                obj_boxes = boxes['obj']
                list_img = aug_transform(list_img, boxes=obj_boxes)
            else:
    ...
    def get_frame_path(self, vid_name, frame_idx):
        ...
        ppath = os.path.join(self.data_root, 'frames/fps_12', vid_name, '%04d.jpg' % (frame_idx + 1))
        # ppath = os.path.join(self.data_root, 'frames', vid_name, '%s_%06d.jpg' % (vid_name, frame_idx + 1))
        return ppath
    ...
    def get_boxes_gt(self, index):
        ...
        if self.cfg.HOIVIT.ENABLE or self.cfg.STDVIT.ENABLE or self.cfg.UNIONHOIVIT.ENABLE:
            box_tensors = {}
            box_tensors['obj'] = torch.zeros((self.coord_nr_frames, self.cfg.HOIVIT.O, 4), dtype=torch.float32)  # (cx, cy, w, h)
            box_tensors['hand'] = torch.zeros((self.coord_nr_frames, self.cfg.HOIVIT.U, 4), dtype=torch.float32)  # (cx, cy, w, h)
            box_categories = torch.zeros((self.coord_nr_frames, self.cfg.HOIVIT.O + self.cfg.HOIVIT.U), dtype=torch.long)
        else:
            ...
            for box_data in frame_data['labels']:
                ...
                if self.cfg.HOIVIT.ENABLE or self.cfg.STDVIT.ENABLE or self.cfg.UNIONHOIVIT.ENABLE:
                    ...
                    try:
                        ...
                        if box_data['standard_category'] == 'hand':
                            box_tensors['hand'][frame_index, global_box_id] = torch.tensor(gt_box).float()
                        else:
                            box_tensors['obj'][frame_index, global_box_id] = torch.tensor(gt_box).float()
