...
def construct_optimizer(model, cfg):
    ...
    hoivit_bn_parameters = []
    hoivit_non_bn_parameters = []
    hoivit_zero_parameters = []
    hoivit_no_grad_parameters = []
    ...
    for name, p in model.named_parameters():
        ...
        if not p.requires_grad:
            ...
            elif 'hoivit' in name: hoivit_no_grad_parameters.append(p)
            else: no_grad_parameters.append(p)
        elif is_bn:
            if 'orvit' in name: orvit_bn_parameters.append(p)
            elif 'hoivit' in name: hoivit_bn_parameters.append(p)
            ...
            elif 'hoivit' in name: hoivit_zero_parameters.append(p)
            else: zero_parameters.append(p)

        else:
            if 'orvit' in name: orvit_non_bn_parameters.append(p)
            elif 'hoivit' in name: hoivit_non_bn_parameters.append(p)
    ...
    elif cfg.SOLVER.HOIVIT_BASE_LR > 0:
        optim_params = [
            {"params": bn_parameters, "weight_decay": cfg.BN.WEIGHT_DECAY, "is_hoivit": False},
            {"params": non_bn_parameters, "weight_decay": cfg.SOLVER.WEIGHT_DECAY, "is_hoivit": False},
            {"params": zero_parameters, "weight_decay": 0.0, "is_hoivit": False},
            {"params": hoivit_bn_parameters, "weight_decay": cfg.BN.WEIGHT_DECAY, 'lr':cfg.SOLVER.HOIVIT_BASE_LR, "is_hoivit": True},
            {"params": hoivit_non_bn_parameters, "weight_decay": cfg.SOLVER.WEIGHT_DECAY, 'lr':cfg.SOLVER.HOIVIT_BASE_LR, "is_hoivit": True},
            {"params": hoivit_zero_parameters, "weight_decay": 0.0, 'lr':cfg.SOLVER.HOIVIT_BASE_LR, "is_hoivit": True},
        ]
    
    else:
        optim_params = [
            {"params": bn_parameters + orvit_bn_parameters + hoivit_bn_parameters, "weight_decay": cfg.BN.WEIGHT_DECAY},
            {"params": non_bn_parameters + orvit_non_bn_parameters + hoivit_non_bn_parameters, "weight_decay": cfg.SOLVER.WEIGHT_DECAY},
            {"params": zero_parameters + orvit_zero_parameters + hoivit_zero_parameters, "weight_decay": 0.0},
        ]
    ...
    assert len(list(model.parameters())) == sum([
        ...
        len(hoivit_non_bn_parameters),
        len(hoivit_bn_parameters),
        len(hoivit_zero_parameters),
        len(hoivit_no_grad_parameters),]), "parameter size does not match: {} + {} + {} + {} != {}".format(
        len(non_bn_parameters),
        len(bn_parameters),
        len(zero_parameters),
        len(no_grad_parameters),
...
def set_lr(optimizer, new_lr, log = False):
    ...
    elif 'hoivit_lr' in new_lr:
        n_hoivit = 0
        n_p_hoivit = 0
        n_p = 0
        n = len(optimizer.param_groups)
        for param_group in optimizer.param_groups:
            if param_group['is_hoivit']:
                n_hoivit += 1
                n_p_hoivit += len(param_group['params'])
            else:
                n_p += len(param_group['params'])
            _new_lr = new_lr['hoivit_lr'] if param_group['is_hoivit'] else new_lr['lr']
            param_group["lr"] = _new_lr
        if log: logger.info(f"Set lr {new_lr['lr']} for {n - n_hoivit} groups parameters with {n_p} parameters and {new_lr['hoivit_lr']} for {n_hoivit} hoivit group parameters with {n_p_hoivit} parameters")
