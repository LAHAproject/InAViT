...
from slowfast.models.attention import TrajectoryAttention, SelfAttentionBlock
...
class ORViT(nn.Module):
    ...
    def forward(self,x, metadata,thw):
        ...
        box_tensors = metadata['orvit_bboxes']['obj'][:,:,:self.cfg.ORVIT.O,:]
        ...
        box_emb = self.c_coord_to_feature(box_tensors.float())
    ...
    def __init__(self, cfg, dim=768, num_heads=12, attn_type='trajectory', 
        ...
        if self.cfg.ORVIT.ENABLE:
            if self.cfg.ORVIT.MOTION_STREAM_SEP_POS_EMB:
                ...
                self.box_categories = nn.Parameter(torch.zeros(self.nb_frames, self.cfg.ORVIT.O, self.in_dim))
        if self.cfg.HOIVIT.ENABLE or self.cfg.STDVIT.ENABLE:    
            if self.cfg.HOIVIT.HANDONLY or self.cfg.STDVIT.HANDONLY:
                self.box_categories = nn.Parameter(torch.zeros(self.nb_frames, self.cfg.HOIVIT.U, self.in_dim))
            elif self.cfg.HOIVIT.OBJONLY or self.cfg.STDVIT.HANDONLY:
