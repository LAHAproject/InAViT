...
from slowfast.utils.meters import AVAMeter, EpochTimer, TrainMeter, ValMeter, eval_extra_metrics,  EPICTrainMeter, EPICValMeter, EGTEATrainMeter, EGTEAValMeter
    ...
    for cur_iter, (inputs, labels, _vid_idx, meta) in enumerate(train_loader):
        ...
        with torch.cuda.amp.autocast(enabled=cfg.TRAIN.MIXED_PRECISION):
            ...
            if cfg.TRAIN.DATASET == 'epickitchens':
                ...
                loss_dict = loss_fun(extra_preds, labels)
                loss = loss_dict['verb_loss'] + loss_dict['noun_loss'] + loss_dict['action_loss']
            else:
        ...
        if cfg.DETECTION.ENABLE:
            ...
            if isinstance(labels, (dict,)) and (cfg.TRAIN.DATASET == "epickitchens" or cfg.TRAIN.DATASET == "egtea"):
                ...
                if cfg.TRAIN.DATASET == "egtea":
                    loss_verb, loss_noun, loss_action = loss_dict['verb_loss'], loss_dict['noun_loss'], loss_dict['action_loss']
                ...
                if cfg.TRAIN.DATASET == "egtea":
                    ...
                    action_top1_acc, action_top5_acc = metrics.topk_accuracies(
                        extra_preds['action'], labels['action'], (1, 5))
                    
                    # Gather all the predictions across all the devices.
                    if cfg.NUM_GPUS > 1:
                        loss_action, action_top1_acc, action_top5_acc = du.all_reduce(
                        [loss_action, action_top1_acc, action_top5_acc]
                        )
                ...
                if cfg.TRAIN.DATASET == "egtea":
                    ...
                    train_meter.update_stats(
                        (verb_top1_acc, noun_top1_acc, action_top1_acc),
                        (verb_top5_acc, noun_top5_acc, action_top5_acc),
                        (loss_verb, loss_noun, loss_action, loss),
                        lr, inputs[0].size(0) * cfg.NUM_GPUS
                    )
            else:
...
def eval_epoch(val_loader, model, val_meter, cur_epoch, cfg, writer=None):
    ...
    for cur_iter, (inputs, labels, _, meta) in enumerate(val_loader):
        ...
        if cfg.DETECTION.ENABLE:
            ...
            preds = model(inputs, meta, cur_epoch, bboxes=meta["boxes"])
            if isinstance(preds, tuple): preds, extra_preds = preds
...
def train(cfg):
    ...
    if cfg.DETECTION.ENABLE:
        ...
        if cfg.TRAIN.DATASET == 'epickitchens':
            ...
            train_meter = EGTEATrainMeter(len(train_loader), cfg)
            val_meter = EGTEAValMeter(len(val_loader), cfg)
        else:
            train_meter = TrainMeter(len(train_loader), cfg)
            val_meter = ValMeter(len(val_loader), cfg)
